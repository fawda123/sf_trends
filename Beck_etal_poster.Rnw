...
png('posterfigs/stations.png', height = 4, width = 7.75, units = 'in', res = 600, family = 'serif')
grid.arrange(pdelta, pinset, ncol = 2, widths = c(1, 0.78))
...
@	    
			...
			\begin{itemize}
	    \item \alert{Nine nutrient stations} with bimonthly samples and daily \alert{flow estimates} from major inflows used for analysis
	    \end{itemize}
	    \begin{figure}
      \centerline{\includegraphics[width=0.95\linewidth]{posterfigs/stations.png}}
      \caption{\footnotesize Locations of bimonthly nutrient (green) and daily flow (red) stations in SFE.}
	    \end{figure}
	    \vspace{-2cm}
			\end{block}
...
png('posterfigs/mopanels.png', height = 5, width = 10.5, units = 'in', res = 600, family = 'serif')
grid.arrange(
  arrangeGrob(pdynaleg, pfitmleg, ncol = 2), 
  pdyna, pfitm, left = textGrob(ylab, rot = 90), 
  heights = c(0.2, 1.1, 1)
  )
dev.off()
...
png('posterfigs/prdseas.png', height = 6, width = 5, units = 'in', res = 600, family = 'serif')
grid.arrange(
  arrangeGrob(pprdnleg, pseasleg, ncol = 2), 
  pA, pB, left = textGrob(ylab, rot = 90), 
  heights = c(0.2, 1, 1.1)
  )
dev.off()
@   
...
pdin <- prdnrmplot(diat_dat$mod[[1]], col_vec = cols, logspace = F) + #, tau = 0.5) +
  theme(legend.position = 'none', axis.text.x = element_blank(),
    panel.background = element_rect(fill = alpha(cols[3], 0.07))) +
  scale_y_continuous(ylab1)

pchl <- prdnrmplot(diat_dat$mod[[4]], col_vec = cols, logspace = F) + #, tau = 0.5) +
  theme(legend.position = 'none', axis.text.x = element_blank(),
    panel.background = element_rect(fill = alpha(cols[3], 0.07))) + 
  scale_y_continuous(ylab2)

psio <- prdnrmplot(diat_dat$mod[[5]], col_vec = cols, logspace = F) + #, tau = 0.5) +
...
png('posterfigs/d7trnds.png', height = 5, width = 8, units = 'in', res = 600, family = 'serif')
grid.arrange(pA, pB, pC, ncol = 1, heights = c(0.85, 0.85, 1))
dev.off()
...
@
        ...
        \begin{varblock}[0.96\textwidth]{Further Evaluation}
          ...
          \begin{figure}
          \centerline{\includegraphics[width=0.85\linewidth]{posterfigs/d7trnds.png}}
