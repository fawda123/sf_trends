...
@
        ...
        \item \alert{Flow-normalized results} can more clearly describe trends independent of confounding variables, e.g., water withdrawals, climatic variation, or management changes
        \item Conditional quantile distributions can reveal changes over time in \alert{high or low nutrient events}, changes that often differ from the median response
        \end{itemize}
        \end{varblock}
        \small{
