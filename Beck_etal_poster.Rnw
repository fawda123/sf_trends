...
grid.arrange(pinset, pdelta, ncol = 2, widths = c(0.78, 1))
...
grid.arrange(
  arrangeGrob(pdynaleg, pfitmleg, pprdnleg, ncol = 3, widths = c(1, 1, 0.6)), 
  arrangeGrob(
...
@   
      ...
      WRTDS models were applied to \alert{nutrient} observations in relation to \alert{time, discharge, and season}: 
	    ...
	    \begin{column}{0.53\linewidth}
	      WRTDS output showed \alert{seasonal variation}, response to \alert{flow changes}, and different \alert{quantile distributions}
	      \begin{figure}
...
@
      ...
      Results for \alert{all Delta stations} and three \alert{nitrogen analytes} were used to evaluate \alert{annual and monthly trends} over time and space
      \vspace{-1.5cm}	
      \begin{columns}[t]
...
@
        ...
        \begin{varblock}[0.96\textwidth]{Further Evaluation}
          ...
          \caption{\footnotesize Flow-normalized trends of annual (left) and seasonal (right) variation in DIN, Chl-{\it\footnotesize a}, and SiO$^2$ at D7.  RGB colors indicate a unique year from 1976 to 2012.}
    	    \end{figure}	
        \end{varblock}
        ...
        \begin{itemize}
        \item WRTDS analyses on four decades of nutrient data in SFE can reveal \alert{undescribed spatiotemporal variation}
        \item \alert{Flow-normalized results} can describe trends independent of confounding variables
        \item Conditional quantile distributions can reveal changes over time in \alert{high or low nutrient events}
        \end{itemize}
        \end{varblock}
        \small{
        We acknowledge the significant efforts of California DWR staff in providing access to quality nutrient and flow data. 
