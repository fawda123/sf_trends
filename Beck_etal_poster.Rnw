...
pinset <- ggplot(sf_bay, aes(x = long, y = lat)) + 
  ...
  geom_point(data = data.frame(x = -121.94, y = 38.07), aes(x, y), fill = cols[2], size = 5, pch = 21) +
...
@	    
			...
			\begin{itemize}
	    \item Nine nutrient stations with bimonthly samples and daily flow estimates from major inflows
	    \end{itemize}
	    \begin{figure}
...
cap <- '\\footnotesize Percent changes in \\alert{DIN} by annual and monthly periods.' 
...
cap <- '\\footnotesize Percent changes in \\alert{ammonium} by annual and monthly periods.'
suppressWarnings(print.xtable(xtable(tab, caption = cap, align = 'llRRRRRR', digits = 1), type = 'latex', 
	include.rownames = F, caption.placement = 'top', size = '\\scriptsize', sanitize.text.function = function(x){x},
	tabular.environment = 'tabularx',
	booktabs = T, width = '0.95\\textwidth'))
...
cap <- '\\footnotesize Percent changes in \\alert{nitrite/nitrate} by annual and monthly periods.' 
suppressWarnings(print.xtable(xtable(tab, caption = cap, align = 'llRRRRRR', digits = 1), type = 'latex', 
	include.rownames = F, caption.placement = 'top', size = '\\scriptsize', sanitize.text.function = function(x){x},
	tabular.environment = 'tabularx',
	booktabs = T, width = '0.95\\textwidth'))
